...
export function ScrollProgress() {
  ...
  // Neural network nodes along the progress bar
  const neuralNodes = useMemo(() => 
    Array.from({ length: 8 }, (_, i) => ({
      id: i,
      position: (i + 1) * 12.5, // Distribute evenly across 100%
      delay: i * 0.1
    })), []
  )
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.on('change', (latest) => {
      // Show progress bar after scrolling 3%
      ...
      if (latest < 0.15) setCurrentSection('NEURAL_INITIALIZATION')
      else if (latest < 0.3) setCurrentSection('SYSTEM_ANALYSIS')
      else if (latest < 0.45) setCurrentSection('DATA_PROCESSING')
      else if (latest < 0.6) setCurrentSection('SKILL_MATRIX')
      else if (latest < 0.75) setCurrentSection('PROJECT_ARCHIVES')
      else if (latest < 0.9) setCurrentSection('COMMUNICATION_LINK')
      else setCurrentSection('TRANSMISSION_COMPLETE')
    })
